// ─────────────────────────────────────────────────────────────
// categories.js — Responsabilidad: CRUD de categorias y
// renderizado de la tabla de presupuesto y barras comparativas
// ─────────────────────────────────────────────────────────────
  ...
  tbody.innerHTML = state.categories.map(cat => {
    ...
    return `<tr>
      <td><div class="cat-name-cell">
        ...
        <input class="cat-name-input" value="${escHtml(cat.name)}"
          oninput="state.categories.find(c=>c.id===${cat.id}).name=this.value; saveState(); fillCatSelect()">
      </div></td>
      <td class="right">
...
function updateSummaryCards() {
  ...
  _set('p-plan-pct',    income>0 ? (totalPlan/income*100).toFixed(1)+'% del ingreso' : '–');
  _set('p-real',        fmt(totalReal));
  _set('p-real-pct',    income>0 ? (totalReal/income*100).toFixed(1)+'% del ingreso' : '–');
  _set('p-savings',     fmt(savings));
  _set('p-savings-pct', income>0 ? (savings/income*100).toFixed(1)+'% tasa de ahorro' : '–');
  ...
  _set('sg-plan-pct',  income>0 ? (totalPlan/income*100).toFixed(1)+'% del ingreso' : '–');
  _set('sg-real',      fmt(totalReal));
  _set('sg-real-pct',  income>0 ? (totalReal/income*100).toFixed(1)+'% del ingreso' : '–');
  _set('sg-avail',     fmt(avail));
  _set('sg-avail-pct', avail >= 0 ? 'disponible del presupuesto' : 'por encima del presupuesto');
