// expenses.js — English version
// Responsibility: individual expense CRUD and rendering of the
...
function renderExpenses() {
  ...
  tbody.innerHTML = exps.map(e => {
    ...
    const name    = cat ? escHtml(getDisplayName(cat)) : 'Unknown';
...
function fillCatSelect() {
  ...
  sel.innerHTML = state.categories.map(c =>
    `<option value="${c.id}" ${c.id == cur ? 'selected' : ''}>${escHtml(getDisplayName(c))}</option>`
  ).join('');
}
